...
var modules,
	...
	jsx = window.jsx = function (called) {
		var base;
		var $called = called;
		if (typeof called === 'string') {
			base = $('[name=jsx][called='+called+']').not('[name=jsx] *').get(0);
			if (!base) return false;
		}
		if (typeof base !== 'object') return false;

		var jsxDefs = jsx.prototype.parseTmpl(base);

//		var jsxDefs = jsx.prototype.locate_templates(called);

		var $jsx = eval(jsx.inst.toSource);
		$jsx.prototype = eval(jsx.prototype.toSource()); //jsx.prototype;
		jsx = $jsx;
		jsx.prototype = $jsx.prototype;
	...
	var $jsx = window.jsx = window.$jsx = jsx;

	jsx.prototype = {
		catagory: 0,
		_version: "0.3.0",

		getBody: function () { return this.tmpl; },
		getCatagory: function () { return this.catagory; },
		...
		*/
		...
		parse_events: function (eventNames) {
		...
		*/
		...
		getInstancesByName: function(called) {
		...
		get_active_attributes: function () {
		...
		static_attribute: function (attr) {
		...
		parseTmpl: function (base) {
			var list = $(base).find('[name=jsx]').get();
			var defs = {};
			for (var i = 0; i < list.length; i++) {
				...
				if (this.util.isset(list[i], 'jsxFound')) continue;
				if (!this.util.istype('object', defs, name)) defs[name] = new Array();
				defs[name].push(list[i]);
			}
			return defs;
		},

		locate_templates: function (list) {
			list = list || this.getJSXElements();
			for (var i = 0; i < list.length; i++) {
				if (this.util.isset(list[i], 'jsxFound')) continue;
				var name = list[i].getAttribute('called');
				var last = list[i].parentNode;
		...
		stack_scope: function (obj) {
			...
			for (var i=0; i < obj.length; i++)
				this.scope.push(obj[i]);
//				this.scope.push(obj.slice(0));
...
//			this.self.stdargs.push(args.slice(i));
		...
		make_actions: function () {
			...
			for (var i = 1; i < this.scope.length; i++)
				scopeStr += 'var $'+i+'_ = this.scope['+i+"]\n";
			eval(scopeStr);
			var $_ = this.self;
			this.shebang = function (cmd) {
		...
		destroy_children: function (name) {
			var list;
			switch (typeof(name)) {
				case ('string'):
				case ('number'):
